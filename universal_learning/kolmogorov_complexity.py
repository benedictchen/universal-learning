"""
🔢 Kolmogorov Complexity - The Universal Measure of Information
==============================================================
...
🎯 ELI5 Summary:
Imagine you want to measure how "complex" or "random" something really is. Is a 
string of a million zeros complex? What about a sequence of perfectly random numbers?

Kolmogorov complexity asks: "What's the shortest computer program that could generate 
this data?" A million zeros can be generated by a tiny program: "print '0' a million 
times" - so it has low complexity despite being long! But truly random data needs a 
program as long as the data itself.

Think of it like compression - highly compressible data has patterns (low complexity),
while incompressible data is essentially random (high complexity). It's the ultimate
measure of information content - how much "surprise" is really in your data!

🧪 Research Background:
The concept emerged independently in the 1960s from three brilliant minds working on 
the fundamental nature of information and randomness. Their work laid the foundation 
for algorithmic information theory and revolutionized our understanding of complexity.

Key Historical Insights:
- Solomonoff (1964): Invented algorithmic probability for inductive inference
- Kolmogorov (1965): Defined complexity as shortest program length
- Chaitin (1966): Connected complexity to incompleteness and randomness
- Martin-Löf (1966): Developed algorithmic tests for randomness
- Li & Vitányi (1997): Comprehensive theory and practical applications

The profound insight: complexity isn't about size or appearance - it's about the 
shortest description. This connected computer science to physics (thermodynamic 
entropy), mathematics (algorithmic randomness), and philosophy (the nature of 
information itself).

🔬 Mathematical Framework:
Kolmogorov Complexity:
K(x) = min{|p| : U(p) = x}
(shortest program p on universal Turing machine U that outputs x)

Conditional Complexity:
K(x|y) = min{|p| : U(p,y) = x}
(shortest program that outputs x given y as input)
...
P(x) ≈ 2^(-K(x))
(probability that random program generates x)

Normalized Compression Distance:
NCD(x,y) = (C(xy) - min(C(x),C(y))) / max(C(x),C(y))
where C(x) is compressed size of x

🎨 ASCII Diagram - Kolmogorov Complexity Spectrum:
================================================

    LOW COMPLEXITY ←──────────────────────→ HIGH COMPLEXITY
    (Highly Structured)                    (Highly Random)
    
    ┌─────────────────┐  ┌─────────────────┐  ┌─────────────────┐
    │ "000000000000"  │  │ "π digits"      │  │ "101011001110"  │
    │                 │  │                 │  │                 │
    │ Program:        │  │ Program:        │  │ Program:        │
    │ for i in range  │  │ def compute_pi  │  │ print "1010110  │
    │  (12):          │  │   # complex     │  │   01110"        │
    │   print('0')    │  │   # algorithm   │  │                 │
    │                 │  │   return digits │  │ (no shorter     │
    │ Length: ~30     │  │ Length: ~100    │  │  program)       │
    │ Data: 12 chars  │  │ Data: 1000 dig  │  │ Length: ~12     │
    │                 │  │                 │  │ Data: 12 chars  │
    │ Ratio: 0.4      │  │ Ratio: 0.1      │  │ Ratio: 1.0      │
    └─────────────────┘  └─────────────────┘  └─────────────────┘
            ↓                       ↓                       ↓
      COMPRESSIBLE            SEMI-COMPRESSIBLE        INCOMPRESSIBLE
      
    COMPRESSION APPROXIMATION:
    =========================
    
    Original Data                     Compressed Data               K(x) Estimate
    ┌─────────────────┐              ┌─────────────────┐          ┌─────────────┐
    │ Text File       │              │ ZIP File        │          │ Complexity  │
    │                 │   ──────→    │                 │  ───→    │             │
    │ "aaaabbbbcccc"  │   compress   │ "4a4b4c"        │  ratio   │ K(x) ≈ 7    │
    │ Size: 12 bytes  │              │ Size: 7 bytes   │          │ (low)       │
    └─────────────────┘              └─────────────────┘          └─────────────┘
    
    ┌─────────────────┐              ┌─────────────────┐          ┌─────────────┐
    │ Random Data     │              │ ZIP File        │          │ Complexity  │
    │                 │   ──────→    │                 │  ───→    │             │
    │ "xk9v2mq8w7s1"  │   compress   │ "xk9v2mq8w7s1"  │  ratio   │ K(x) ≈ 12   │
    │ Size: 12 bytes  │              │ Size: 12 bytes  │          │ (high)      │
    └─────────────────┘              └─────────────────┘          └─────────────┘

    APPLICATIONS WORKFLOW:
    ====================
    
    Data Source → Complexity Analysis → Application
    
    ┌─────────────┐    ┌─────────────┐    ┌─────────────────────┐
    │ Text Files  │    │ Plagiarism  │    │ Academic Integrity  │
    │ Documents   │───→│ Detection   │───→│ Content Analysis    │
    │ Web Pages   │    │ via NCD     │    │ Similarity Metrics  │
    └─────────────┘    └─────────────┘    └─────────────────────┘
    
    ┌─────────────┐    ┌─────────────┐    ┌─────────────────────┐
    │ DNA Seqs    │    │ Evolution   │    │ Species Phylogeny   │
    │ Proteins    │───→│ Analysis    │───→│ Biological Classify │
    │ Genomes     │    │ via MDL     │    │ Mutation Detection  │
    └─────────────┘    └─────────────┘    └─────────────────────┘
    
    ┌─────────────┐    ┌─────────────┐    ┌─────────────────────┐
    │ Images      │    │ Pattern     │    │ Computer Vision     │
    │ Audio       │───→│ Discovery   │───→│ Anomaly Detection   │
    │ Time Series │    │ Clustering  │    │ Compression Codecs  │
    └─────────────┘    └─────────────┘    └─────────────────────┘

💡 Applications:
- Data compression and information theory
- Plagiarism detection and document similarity
- Bioinformatics (DNA analysis, phylogenetic reconstruction)  
...
Kolmogorov complexity is one of the deepest concepts in computer science - it unifies
information theory, computability theory, and the philosophy of randomness into a single
elegant framework. Every bit of information has its shortest description!

🔗 Related: Algorithmic Information Theory, Minimum Description Length, Data Compression, Randomness
"""
